...
st.subheader("Архитектура оптимизатора")
...
* используется предварительно обученная нейронная сеть для предсказания массы и жесткости *(метамодель)*
* градиентная оптимизация: алгоритм *Adam* для поиска оптимальных параметров

2. Формула функции потерь """)
st.latex(r'''Loss = -EJ_{xi} + penalty,''')

st.markdown("""где $-EJ_{xi}$ - максимизация жесткости;  
...
st.markdown(""" где  $m_{предсказанная}$ - предсказанное метамоделью значение массы;  
...
* технологические - округление до ближайшего допустимого значения из технологического списка 
***0.8, 0.9, 1.0, 1.2, 1.5, 1.6, 1.8, 1.9, 2.0, 2.5*** *мм*

""")
with st.expander("Код оптимизатора"):
    code = '''#обертка оптимизатора
